...
public class Inventory {
					...
					if(buyOrSell) {	
						...
						try {
							...
							//	inventory[itemIndex].updateItem(quantity);
								...
								inventory.get(itemIndex).updateItem(quantity);
						...
						try {
							...
							if(quantity > 0) {
								...
								boolean upresult = inventory.get(itemIndex).updateItem(quantity * -1);
	...
	public void toLinkedList() {
		LinkedList<FoodItem> inventoryList = new LinkedList<FoodItem>();
		
		//add each item of array to the Linkedlist
		for(int i =0 ; i < inventory.size(); i++){
		...
		String output = "Inventory in Linked List: \r\n";
		
		//only print if there are items
		if(numItems > 0) {
			//for loop to go through each item, add to output string
			for(int i = 0; i < numItems; i++) {
				output += inventoryList.get(i).toString() + "\r\n";
			}//end for
		}//end if
